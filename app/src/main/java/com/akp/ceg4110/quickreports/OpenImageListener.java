...
public class OpenImageListener implements View.OnClickListener{
    ...
    public static void layoutImages(int height, int width, Bitmap imageBitmap, GridLayout theImagesLayout, ImageView theImage){
        ...
        if(imageBitmap.getHeight() == Math.max(imageBitmap.getHeight(), imageBitmap.getWidth())){   //If taller
            ...
            imageBitmap = Bitmap.createScaledBitmap(imageBitmap, width / gridSize - 20,
                                                    imageBitmap.getHeight() / (imageBitmap.getWidth() / (width / gridSize - 20)),
                                                    false);
        }else{  // If wider than tall
            // Same process as above, but using height as the reference and scaling width accordingly
            imageBitmap = Bitmap
                    .createScaledBitmap(imageBitmap, imageBitmap.getWidth() / (imageBitmap.getHeight() / (width / gridSize - 20)),
                                        width / gridSize - 20, false);
        }
        ...
        imageBitmap = Bitmap.createBitmap(imageBitmap, (imageBitmap.getWidth() - (width / gridSize - 20)) / 2,
                                          (imageBitmap.getHeight() - (width / gridSize - 20)) / 2, width / gridSize - 20, width / gridSize - 20);
        ...
        theImage.setMaxWidth(width / gridSize - 20);    // Show images at 1/3rd the size for three columns - see above
